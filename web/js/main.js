            ...
            window.viewer.entities.add({
                ...
                description: `
                        <div id="tablaProperties${feature.properties.id}"  class="tablePorperties" >
                            ...
                            <p>TLS ID: ${feature.properties.tls_id}</p>
                            <button id="${feature.properties.id}" class="cesium-button">
                                Prohibir ⛔
                            </button>
                        </div>
                    `
        ...
        async function getRoadsWebsocket(){
            ...
            socket = new WebSocket(endPoint+'ws/getRoads?bbox='+JSON.stringify(payload));
            socket.onopen = () => {
        ...
        async function runSimulationWebsocket() {
            ...
            socket = new WebSocket(endPoint+'ws/simulation?bbox='+JSON.stringify(payload)+"&forbiddenRoads="+forbiddenRoads+"&num_vehicles="+num_vehicles+"&duration_sec="+duration_sec);
        ...
        function testConnectionProxy(){
            ...
            // Ahora (vía Nginx):
            socket = new WebSocket(endPoint + 'ws/status');
